...
const grid = [...5].map(() => [...5].map(n => generateFruit()))
let score = 0
...
for (const [addX, addY] of directions) if (grid[y + addY]?.[x+addX] === type && !checked(x + addX, y+addY)) res.push(...around(x+addX,y+addY, res.concat(ignore)))
return res
}
...
if (near.length > 2) {
for (const [popX, popY] of near.sort((a,b) => a[1] - b[1])) pop(popX, popY)
